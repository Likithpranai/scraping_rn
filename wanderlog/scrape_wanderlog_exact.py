...
async def scrape_bar_details(page, bar_url, bar_name, ranking):
    ...
    try:
        ...
        # 6. source_photoUrls - get all image URLs with class="w-100 h-100 object-fit-cover"
        source_photoUrls = []
        ...
        # 7. source_categories - get all badge content from div with class="d-flex flex-row align-items-center mt-2 flex-wrap"
        source_categories = []
...
async def main():
    ...
    if bars_data:
        ...
        # Save to JSON file
        output_file = os.path.join('wanderlog', 'hong_kong_bars_exact.json')
